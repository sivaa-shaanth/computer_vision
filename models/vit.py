""" Vision Transformers as described in:
...
default_cfgs = {
    ...
    "vit_tiny_patch16_224": _cfg(url=""),
    "vit_small_patch16_224": _cfg(url=""),
    "vit_base_patch32_224": _cfg(url=""),
    "vit_base_patch16_224": _cfg(url=""),
    "vit_base_patch8_224": _cfg(url=""),
}
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = ""):
    ...
    if pos_embed_w.shape != model.pos_embed.shape:
        pos_embed_w = (
            resize_pos_embed(  # resize pos embedding when different size from pretrained weights
                pos_embed_w,
                model.pos_embed,
...
@register_model
def vit_tiny_p16(pretrained=False, **kwargs):
    """ViT-Tiny (Vit-Ti/16)"""
...
@register_model
def vit_small_p16(pretrained=False, **kwargs):
    """ViT-Small (Vit-Si/16)"""
...
@register_model
def vit_base_p16(pretrained=False, **kwargs):
    """ViT-Base (Vit-Bi/16)"""
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=24, **kwargs)
    model = _create_vision_transformer(
